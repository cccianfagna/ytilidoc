 ...
 */
public class PermutationBasedOnOperations {
    ...
    private static int[] solution(int N, int[] A) {
        ...
        for (int i = 0; i < A.length; i++) {
            if (A[i] >= 1 && A[i] <= N) {
                counters[A[i]-1] = Math.max(counters[A[i]-1], previousMax);
                counters[A[i]-1] += 1;
                currentMax = Math.max(counters[A[i]-1], currentMax);
            } else if (N + 1 == A[i]) {
    ...
    private static int[] bruteForceSolution(int N, int[] A) {
        int maxValue = 0;
        ...
        for (int i = 0; i < A.length; i++) {
            if (A[i] >= 1 && A[i] <= N) {
                integers[A[i]-1] += 1;
                maxValue = Math.max(maxValue, integers[A[i]-1]);
            }
            if (A[i] == N + 1) {
                for(int j = 0; j < N; j++) {
                    integers[j] = maxValue;
